...
impl<'s> System<'s> for ProjectileSystem {
    ...
    fn run(&mut self, (entities, projectiles, mut transforms, time): Self::SystemData) {
        for (projectile_entity, projectile_component, projectile_transform) in
            (&*entities, &projectiles, &mut transforms).join()
        {
            projectile_transform.translate_y(
                projectile_component.rise
                    * GAME_CONFIGURATION.projectile_velocity
                    * time.delta_seconds(),
            );
            projectile_transform.translate_x(
                projectile_component.run
                    * GAME_CONFIGURATION.projectile_velocity
                    * time.delta_seconds(),
            );
