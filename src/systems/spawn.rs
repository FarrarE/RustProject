use amethyst::{
    ...
    ecs::prelude::{Entities, Entity, LazyUpdate, Read, ReadExpect, System, WriteExpect},
};
use rand::Rng;
...
impl<'s> System<'s> for SpawnSystem {
    ...
    fn run(&mut self, (entities, mut enemy_resource, time, lazy_update): Self::SystemData) {
            ...
            && enemy_resource.num_enemies < GAME_CONFIGURATION.max_monster_count
        {
            ...
            match edge {
                0 => {
                    trans.set_xyz(0.0, location, 0.0);
                }
                1 => {
                    trans.set_xyz(location, ARENA_HEIGHT - enemy_component.height, 0.0);
                }
                2 => {
                    trans.set_xyz(ARENA_WIDTH - enemy_component.width, location, 0.0);
                }
                3 => {
                    trans.set_xyz(location, 0.0, 0.0);
                }
                _ => {}
            }
